...
INFO_MESSAGE = Building full mbed framework
...
#    EXCLUDE_LIST   += $(addprefix %,$(EXCLUDE_NAMES).$(AS_EXTENSION))
endif
...
ifneq ($(wildcard $(CORE_LIB_PATH)/targets/hal/$(LEVEL1)/$(LEVEL2)/Lib),)
    u101   += $(shell find $(CORE_LIB_PATH)/targets/hal/$(LEVEL1)/$(LEVEL2)/Lib -type d)
endif
...
ifneq ($(wildcard $(CORE_LIB_PATH)/targets/cmsis/$(LEVEL1)/$(LEVEL2)/$(TOOLCHAIN)/*.$(AS_EXTENSION)),) # */
...
CORE_OBJ_FILES        = $(CORE_C_SRCS:.c=.c.o) $(CORE_CPP_SRCS:.cpp=.cpp.o) $(CORE_AS_SRCS:.$(AS_EXTENSION)=.$(AS_EXTENSION).o)
...
u204                  = $(foreach dir,$(u201),$(wildcard $(dir)/*.h)) # */
BUILD_APP_H_SRCS      = $(filter-out %/$(EXCLUDE_LIST),$(u203))
...
VARIANT_AS_SRC  = $(wildcard $(VARIANT_PATH)/*.$(AS_EXTENSION)) # */
VARIANT_OBJS    = $(patsubst $(APPLICATION_PATH)/%.$(AS_EXTENSION),$(OBJDIR)/%.$(AS_EXTENSION).o,$(VARIANT_AS_SRC)) # */
...
INCLUDE_PATH += $(sort $(dir $(USER_LIB_H_SRC)))
