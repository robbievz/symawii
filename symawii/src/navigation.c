...
{                               // returns fixedpointnum distance in meters and bearing in fixedpointnum degrees from point 1 to point 2
    ...
    *bearing = FIXEDPOINT90 + lib_fp_atan2(-latdiff, londiff);
    if (*bearing > FIXEDPOINT180)
        *bearing -= FIXEDPOINT360;
    ...
    if (lib_fp_abs(latdiff) + lib_fp_abs(londiff) > 40000L) {   // for big distances, don't shift lat and long.  Instead shift the constant by 10.
