                                  ...
                                  uint32_t u32DutyCycle)
{
    ...
    PWM->PCR |= PWM_PCR_CH0MOD_Msk << (4 * u32ChannelNum);
...
/**
 * @brief This function enable fault break of selected channels
 * @param[in] pwm The base address of PWM module
 * @param[in] u32ChannelMask Thie parameter is not used 
 * @param[in] u32LevelMask Output high or low while fault break occures, each bit represent the level of a channel
 *                         while fault break occures. Bit 0 represents channel 0, bit 1 represents channel 1...
 *                         , bit 6 represnt D6, and bit 7 represents D7
 * @param[in] u32BreakSource Fault brake source, could be one of following source
 *                  - \ref PWM_FB0_EINT0
 *                  - \ref PWM_FB0_ACMP1
 *                  - \ref PWM_FB1_EINT1
 *                  - \ref PWM_FB1_ACMP0 
...
{
    PWM->PFBCON = (u32LevelMask << PWM_PFBCON_PWMBKO0_Pos) | u32BreakSource;
}

/**
 * @brief This function clear fault break flag
 * @param[in] pwm The base address of PWM module
 * @param[in] u32BreakSource This parameter is not used
 * @return None
 * @note After fault break occured, application must clear fault breaj source before re-enable PWM output
 */
 ...
 * @retval 0 Duty interrupt did not occured
 * @retval 1 Duty interrupt occured
 */
 ...
 * @param[in] pwm The base address of PWM module
 * @param[in] u32BreakSource Fault break source, could be either
 ...
 *                  - \ref PWM_PIIR_BKF0_Msk, or
 *                  - \ref PWM_PIIR_BKF1_Msk
 * @return Fault break interrupt flag of specified source
 * @retval 0 Fault break interrupt did not occured
 * @retval 1 Fault break interrupt occured
 */
uint32_t PWM_GetFaultBreakIntFlag (PWM_T *pwm, uint32_t u32BreakSource)
{
    return (PWM->PIIR & u32BreakSource ? 1 : 0);
}
 ...
 * @retval 0 Period interrupt did not occured
 * @retval 1 Period interrupt occured
 */
